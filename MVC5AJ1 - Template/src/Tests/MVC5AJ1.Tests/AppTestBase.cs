...
using MVC5AJ1.Migrations.Seed.Host;
using MVC5AJ1.Migrations.Seed.Tenants;
using MVC5AJ1.MultiTenancy;
using MVC5AJ1.Tests.TestDatas;
...
namespace MVC5AJ1.Tests
{
    ...
    /// Provides methods to easily work with <see cref="MVC5AJ1DbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<MVC5AJ1TestModule>
    {
        ...
        protected void UsingDbContext(Action<MVC5AJ1DbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<MVC5AJ1DbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<MVC5AJ1DbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<MVC5AJ1DbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MVC5AJ1DbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MVC5AJ1DbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<MVC5AJ1DbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MVC5AJ1DbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<MVC5AJ1DbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<MVC5AJ1DbContext, Task<T>> func)
        {
            T result;
